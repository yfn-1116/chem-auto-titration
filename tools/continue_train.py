"""继续训练 Color_Model_ (3分类: wine_red/purple/blue)
...
DATA_DIR = os.path.join(CWD, "data_3cls")
...
def collect_all_data(target_dir):
    ...
    for cls_name in CLASSES:
        os.makedirs(os.path.join(target_dir, cls_name))
    ...
    for source_name, source_path in DATA_SOURCES.items():
        for cls_name in CLASSES:
            ...
            for f in files:
                dst = os.path.join(target_dir, cls_name, f)
                if not os.path.exists(dst):
                    shutil.copy2(os.path.join(src_cls_dir, f), dst)
            total += len(files)
            print(f"  [{source_name}] {cls_name}: {len(files)} 张")
    print(f"  共收集 {total} 张图片")

    actual_total = sum(
        len([f for f in os.listdir(os.path.join(target_dir, cls))
             if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))])
        for cls in CLASSES
    )
    print(f"  去重后共 {actual_total} 张图片")

    # 平衡各类数量，防止 blue 过多压倒 minority 类
    for cls_name in CLASSES:
        ...
        if len(files) > MAX_SAMPLES_PER_CLASS:
            random.seed(42)
            keep = set(random.sample(files, MAX_SAMPLES_PER_CLASS))
            for f in files:
                if f not in keep:
                    os.remove(os.path.join(cls_dir, f))
            print(f"  [平衡] {cls_name}: {len(files)} → {MAX_SAMPLES_PER_CLASS}")
    print(f"  平衡后共 {sum(len(os.listdir(os.path.join(target_dir, cls))) for cls in CLASSES)} 张")
    return actual_total
...
def split_train_val(data_dir, val_ratio=0.15, seed=42):
    ...
    shutil.rmtree(os.path.join(data_dir, CLASSES[0].rsplit("_", 1)[0] if "_" in CLASSES[0] else CLASSES[0]), ignore_errors=True)
    print(f"  分割完成: train/{len(CLASSES)}类, val/{len(CLASSES)}类")

    for cls_name in CLASSES:
        os.rmdir(os.path.join(data_dir, cls_name))
...
def get_transforms():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=0.4, contrast=0.35, saturation=0.3, hue=0.15),
...
def main():
    ...
    sampler = torch.utils.data.WeightedRandomSampler(sample_weights, len(train_labels), replacement=True)
    ...
    pretrained_path = os.path.join(PTHS_DIR, "Color_Model_Net.pth")
    ...
    # 5. 保存类别映射
    json_path = os.path.join(PTHS_DIR, "Color_Model_.json")
    ...
    best_acc = 0.0
    epochs_no_improve = 0
    save_path = os.path.join(PTHS_DIR, "Color_Model_Net.pth")
